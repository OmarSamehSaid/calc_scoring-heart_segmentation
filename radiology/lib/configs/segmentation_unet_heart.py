...
class SegmentationUnetHeart(TaskConfig):
    def __init__(self):
        super().__init__()
    ...
    def init(self, name: str, model_dir: str, conf: Dict[str, str], planner: Any, **kwargs):
        ...
        if strtobool(self.conf.get("use_pretrained_model", "false")):
            ...
            url = f"{url}/radiology_segmentation_unet_heart.pt"
    ...
    def infer(self) -> Union[InferTask, Dict[str, InferTask]]:
        task: InferTask = lib.infers.SegmentationUnetHeart(
            path=self.path,
    ...
    def trainer(self) -> Optional[TrainTask]:
        ...
        task: TrainTask = lib.trainers.SegmentationUnetHeart(
            model_dir=output_dir,
            network=self.network,
            roi_size=self.roi_size,
            target_spacing=self.target_spacing,
            description="Train Unet Heart Segmentation Model",
            load_path=load_path,
    ...
    def scoring_method(self) -> Union[None, ScoringMethod, Dict[str, ScoringMethod]]:
        ...
        if self.epistemic_enabled:
            methods[f"{self.name}_epistemic"] = EpistemicScoring(
                ...
                transforms=lib.infers.SegmentationUnetHeart(None).pre_transforms(),
